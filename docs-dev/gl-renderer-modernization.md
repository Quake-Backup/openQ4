...
The runner covers startup, `gfxInfo`, forced `r_glTier` probes, debug-context fallback, presentation cvar probes, clustered-light diagnostics, deferred-resolve diagnostics, forward+ diagnostics, modern-visible diagnostics, GPU-driven diagnostics, and all renderer foundation self-tests without entering gameplay. The manual section of that document remains the release sign-off matrix for SP/MP map smoke tests once map startup is safe to run.
