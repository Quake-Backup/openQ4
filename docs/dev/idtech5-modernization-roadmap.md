...
| Modern classic-frame ownership | **Experimental; four complete domains, in-world GUI subset, special-subview transaction, cinematic/authored-post transaction, plus material-deform dependency implemented** | Fixed-function root 2D GUI, eligible ambient-only 3D world, fixed-classic interaction, and complete eligible fog/blend-phase ownership have separate default-off shared corridors. A separate default-off in-world GUI corridor owns only the complete provenance-tagged subset emitted by `R_RenderGuiSurf`, with a 3D depth contract and all-tagged-surface classic rollback. The special-subview transaction seals direct `SS_SUBVIEW` mirror semantics plus eligible remote-camera, mirror, reflection, refraction, and x-ray child-scene/copy edges without claiming child material or lighting ownership. The cinematic/authored-post transaction seals an eligible root video/audio view or complete post-process tail before dispatching the established dynamic-stage executor, preserving decoder clock and current-render/depth behavior. The independent default-off material-deform contract distinguishes finalized CPU results from generated/skinned geometry, seals completed or intentional-empty output and classic receiver non-applicability, and lets the shared consumers use ordinary final geometry without rerunning the deform in either backend. Particle/particle2, skip/failure, freshness, cache, or provenance blockers retain complete-domain rollback. Cubemap/depth and unsupported nesting retain classic ownership; clean-package and platform promotion remain open. |
...
in-world GUI subset transaction, a special-subview transaction, a
cinematic/authored-post transaction, and their shared
...
and x-ray views; cubemap/depth and unsupported special nesting retain their
complete classic owners. In-world GUI output from `R_RenderGuiSurf` now has independently
...
implementation target is **Milestone D cubemap/depth special-capture ownership**,
...
| D. Modern classic-frame ownership | **Experimental; four complete domains, in-world GUI subset, special-subview transaction, cinematic/authored-post transaction, special-frame transaction, and material-deform dependency implemented** | Eligible fixed-function root 2D GUI views, ambient-only 3D world views, fixed-classic interaction views, and complete eligible fog/blend phases are transactionally evaluated through backend-neutral sealed records by GL and Vulkan, with complete-domain classic rollback. The default-off in-world GUI corridor transactionally owns the complete provenance-tagged `R_RenderGuiSurf` output subset with 3D depth semantics; one rejection restores all tagged sources to the classic walker. The default-off special-subview transaction seals direct mirror camera/clip/scissor semantics and remote-camera/mirror/reflection/refraction/x-ray capture edges on both backends; cubemap/depth and unsupported special nesting remain classic-owned. The default-off cinematic/authored-post transaction seals eligible complete root video/audio views and post tails before directing the established dynamic stage executors. The default-off special-frame transaction seals active-session render-demo views and exact Raven blur/AL controller coverage while retaining the complete mature executors. A separate default-off deform contract seals the authoritative CPU result, source-preserving interaction/fog roles, frame/cache provenance, intentional empty work, and named unsupported outcomes for the shared consumers. Cubemap/depth special-capture ownership is next. Authored-stock, clean-package, and platform qualification remain open as applicable. |
...
9. **Implemented for special subviews:** a direct `SS_SUBVIEW` mirror record
   ...
   child scene packet, parent source surface, exact full-viewport color
   `RC_COPY_RENDER`, and destination image. Both backends report ownership only
   after the sealed child executor or transfer returns. Cubemap/depth and
   unsupported nested, cinematic, or other special views retain their untouched
   classic owners pending dedicated semantic records.
...
The next implementation target is cubemap/depth special-capture ownership, not
