...
- Replace macOS native window/event pump path with SDL3 equivalents. Source selection now reaches the SDL3 wrapper; experimental macOS CI covers OpenGL and Metal bridge configure/build/install/package validation, and real-device runtime validation remains required for hardware-specific input/audio/display signoff.
