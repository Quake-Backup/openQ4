...
At least one usable `zpak_<language>.pk4` is needed for character dialogue: the shared `pak001.pk4` through `pak022.pk4` baseline contains the sound declarations and lip-sync data, but not the localized voice audio itself. Client startup warns when no recognized retail language-pack filename is mounted. This check identifies the known language name; it does not claim that the archive contents are complete. The absence remains non-fatal so dedicated servers and alternate physical/localized editions are not tied to one specific language archive or checksum.
