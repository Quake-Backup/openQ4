...
- `ClassicInteractionDomain` owns the separate default-off, whole-view unshadowed fixed-classic interaction transaction. Interaction packets carry explicit light and local/global/translucent receiver identity; one bounded preparation walk seals every accepted light stage, decomposed bump/diffuse/specular primitive, no-op, resource, state, and hash. GL/Vulkan preflight the complete sealed view, while every shadow-coupled, custom, deform/skinned, special-view, overflow, or invalid-resource case retains the untouched classic interaction walker.
